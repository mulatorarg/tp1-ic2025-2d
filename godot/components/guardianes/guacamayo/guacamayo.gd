...
@export var nombre : String = "Guacamayo": ## Nombre del Guardián (ave mensajera).
	set (value):
		nombre = value
		if nombre.length() > 0:
			label.text = value
...
@onready var DisparoScene = preload("res://components/guardianes/guacamayo/guacamayo_disparo.tscn")
